...
export const STATE_APPLIERS: Partial<Record<TrekWsTripEventName, StateApplier>> = {
  ...
  'reservation:updated': (payload, state) => {
    ...
    if (!reservation) return {}
    return {
      reservations: state.reservations.map(r => r.id === reservation.id
        ? {
            ...r,
            ...reservation,
            day_positions: reservation.day_positions != null ? reservation.day_positions : r.day_positions,
          }
        : r),
    }
  },
    ...
    return {
      reservations: state.reservations.map(reservation => {
        ...
        if (dayId != null) {
          return {
            ...reservation,
            day_positions: { ...(reservation.day_positions ?? {}), [String(dayId)]: position },
          }
        }
        return { ...reservation, day_plan_position: position }
      }),
    }
  },
...
export function handleRemoteEvent(set: SetState, get: GetState, event: WebSocketEvent): void {
  ...
  if (type === 'accommodation:created' || type === 'accommodation:updated' || type === 'accommodation:deleted') {
    window.dispatchEvent(new CustomEvent('accommodations:refresh'))
  }
