...
function useIntegrations() {
  ...
  const availableOAuthScopes = [
    ...new Set([...ALL_SCOPES, ...oauthPluginResources.flatMap((resource) => resource.scopes)]),
  ];
  ...
  useEffect(() => {
    if (mcpEnabled) {
      ...
      oauthApi
        .pluginResources()
        .then((d) => setOauthPluginResources(d.resources || []))
        .catch(() => setOauthPluginResources([]));
    }
  }, [mcpEnabled]);
            ...
            {!oauthCreatedClient ? (
              <>
                ...
                <div>
                  ...
                  <ScopeGroupPicker
                    selected={oauthNewScopes}
                    onChange={setOauthNewScopes}
                    availableScopes={availableOAuthScopes}
                  />
                </div>
