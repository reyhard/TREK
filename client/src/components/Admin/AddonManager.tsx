            ...
            {integrationAddons.length > 0 && (
              <div>
                ...
                {integrationAddons.map((addon) => (
                  <div key={addon.id}>
                    <AddonRow addon={addon} onToggle={handleToggle} t={t} />
                    {addon.id === 'llm_parsing' && addon.enabled && <LlmParsingConfig addon={addon} />}
                  </div>
                ))}
              </div>
            )}
...
function LlmParsingConfig({ addon }: { addon: Addon }) {
  ...
  const fieldCls =
    'w-full rounded-lg border border-edge-secondary bg-surface px-3 py-2 text-sm text-content placeholder:text-content-faint transition-colors focus:border-edge focus:outline-none';
  ...
  return (
    <div className="border-b border-edge-secondary bg-surface-secondary py-5 pl-[70px] pr-6">
      <div className="max-w-2xl space-y-6">
            ...
            <div className="space-y-3 rounded-lg border border-edge-secondary bg-surface p-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-content-muted underline disabled:opacity-60"
                >
                ...
                <div className="space-y-1">
                  {RECOMMENDED_MODELS.map((m) => {
                    ...
                    return (
                      <div
                        key={m.id}
                        className={`flex items-center gap-3 rounded-lg border px-3 py-2 transition-colors ${active ? 'border-edge-secondary bg-surface-secondary' : 'border-transparent'}`}
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-content">{m.label}</span>
                            {m.recommended && (
                              <span className="rounded-md bg-[rgba(16,185,129,0.15)] px-1.5 py-px text-[10px] font-semibold text-emerald-600">
                                Recommended
                              </span>
                            )}
                          </div>
                        ...
                        {installedHere ? (
                          <button
                            ...
                            disabled={active}
                            className={`shrink-0 rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${active ? 'bg-surface-tertiary text-content-muted' : 'border border-edge-secondary text-content-secondary hover:border-edge'}`}
                          >
                            {active ? 'Selected' : 'Use'}
                          </button>
                        ) : (
                          <button
                            onClick={() => pull(m.id)}
                            disabled={!!pulling}
                            className="shrink-0 rounded-md bg-accent px-3 py-1.5 text-xs font-medium text-accent-text disabled:opacity-60"
                          >
        ...
        <button
          onClick={save}
          disabled={saving}
          className="rounded-lg bg-accent px-4 py-2 text-sm font-medium text-accent-text transition-opacity disabled:opacity-60"
        >
          {saving ? 'Saving…' : 'Save'}
        </button>
      </div>
