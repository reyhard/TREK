import { Car, Footprints, Hotel, TramFront, Zap } from 'lucide-react'
...
export function RouteConnector({
  ...
  useLayoutEffect(() => {
    ...
    const nextPosition = {
      ...
      left: Math.max(viewportPadding, Math.min(triggerBounds.left + triggerBounds.width / 2 - renderedWidth / 2, maximumLeft)),
      width,
    }
    setPosition(current => current.top === nextPosition.top && current.left === nextPosition.left && current.width === nextPosition.width ? current : nextPosition)
  }, [open])
  ...
  const label = (
    <>
      ...
      {seg.noteText && <><span style={{ opacity: 0.4 }}>·</span><span style={{ color: 'var(--text-muted)' }}>{seg.noteText}</span></>}
    </>
  )
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 14px', fontSize: 'calc(10.5px * var(--fs-scale-caption, 1))', color: 'var(--text-faint)', lineHeight: 1.2 }}>
        ...
        {transitAction ? (
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexShrink: 0, minHeight: 24, padding: '3px 7px', margin: '-3px -7px', border: 0, borderRadius: 7, background: open ? 'var(--bg-hover)' : 'transparent', color: open ? 'var(--text-primary)' : 'inherit', cursor: 'pointer', font: 'inherit', lineHeight: 'inherit', transition: 'background 120ms ease, color 120ms ease', position: 'relative', touchAction: 'manipulation', WebkitTapHighlightColor: 'transparent' }}
            onMouseEnter={event => { event.currentTarget.style.background = 'var(--bg-hover)'; event.currentTarget.style.color = 'var(--text-primary)' }}
            onMouseLeave={event => { if (!open) { event.currentTarget.style.background = 'transparent'; event.currentTarget.style.color = 'inherit' } }}
          >{label}</button>
        ) : <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>{label}</div>}
        <div style={line} />
      </div>
      {open && transitAction && ReactDOM.createPortal(
        <div ref={menuRef} role="menu" aria-label={transitAction.label} className="trek-popover-enter" style={{ position: 'fixed', boxSizing: 'border-box', top: position.top, left: position.left, zIndex: 999999, width: position.width, maxWidth: 'calc(100vw - 16px)', padding: 4, border: '1px solid var(--border-primary)', borderRadius: 10, background: 'var(--bg-card)', boxShadow: '0 8px 30px rgba(0,0,0,0.15)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', fontFamily: 'var(--font-system)' }}>
          <button ref={actionRef} type="button" role="menuitem" onClick={() => { setOpen(false); transitAction.onSelect() }} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', minHeight: 36, padding: '8px 10px', border: 0, borderRadius: 7, background: 'transparent', color: 'var(--text-primary)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 'calc(12px * var(--fs-scale-body, 1))', fontWeight: 500, textAlign: 'left' }}>
