  ...
  return (
    ...
    >
      <div className="bg-surface-elevated" style={{
        ...
        {isRepositioning && (
          <div role="status" className="border-t border-edge-faint" style={{ padding: '9px 16px', display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-hover)' }}>
        ...
        <div className="border-t border-edge-faint" style={{ padding: '10px 16px', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }} />
          {mode === 'trip' && canReposition && place.lat != null && place.lng != null && !isRepositioning && (
...
export function ActionButton({ onClick, variant, icon, label, disabled = false, ref }: ActionButtonProps) {
  const base = {
  ...
  return (
    <button type="button"
      ...
      style={{
        ...
        fontSize: 'calc(12px * var(--fs-scale-body, 1))', fontWeight: 500, cursor: disabled ? 'default' : 'pointer',
  ...
  return (
          ...
          {place.route_geometry && (() => {
            try {
              ...
              const hasEle = movement.elevations.some((elevation): elevation is number => elevation != null)

              const distKm = movement.distance / 1000

              // Elevation stats
              const minEle = movement.minElevation ?? Infinity
              const maxEle = movement.maxElevation ?? -Infinity
              const totalUp = movement.elevationGain
              const totalDown = movement.elevationLoss

              // Elevation profile SVG
              const chartW = 280, chartH = 60
              const elevations = hasEle ? movement.elevations.filter((e): e is number => e != null) : []
              let pathD = ''
