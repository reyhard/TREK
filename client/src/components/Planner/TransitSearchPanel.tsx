    ...
    try {
      ...
      transitLegs.slice(0, -1).forEach((leg, i) => {
        ...
        endpoints.push({
          ...
          local_date: s.time ? dateYMDInTz(s.time, tzAt(s.lat, s.lng)) : null,
          local_time: s.time ? timeHHmmInTz(s.time, tzAt(s.lat, s.lng)) : null,
        });
      });
      endpoints.push({
        role: 'to',
        ...
        metadata: {
          transit: {
            ...
            legs: it.legs.map((l) => ({
              ...
              from: {
                name: l.from.name,
                time: l.from.time ? timeHHmmInTz(l.from.time, tzAt(l.from.lat, l.from.lng)) : null,
                track: l.from.track,
              },
              to: {
                name: l.to.name,
                time: l.to.time ? timeHHmmInTz(l.to.time, tzAt(l.to.lat, l.to.lng)) : null,
                track: l.to.track,
              },
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <div className="bg-surface-secondary" style={{ display: 'flex', borderRadius: 9, padding: 3 }}>
              <button onClick={() => setArriveBy(false)} style={segBtn(!arriveBy)}>
                {t('transit.depart')}
              </button>
              <button onClick={() => setArriveBy(true)} style={segBtn(arriveBy)}>
                {t('transit.arrive')}
              </button>
            </div>
            <div style={{ width: 110 }}>
