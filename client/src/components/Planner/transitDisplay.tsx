 ...
 */
export function TransitWalkDivider({
  leg,
  t,
  size = 'md',
}: {
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: size === 'sm' ? '1px 0' : '2px 0' }}>
      ...
      <span
        ...
        style={{
          ...
          gap: 5,
          fontSize:
            size === 'sm' ? 'calc(10px * var(--fs-scale-caption, 1))' : 'calc(11.5px * var(--fs-scale-caption, 1))',
          fontWeight: 500,
          whiteSpace: 'nowrap',
          overflow: 'hidden',
 ...
 */
export function TransitItineraryInline({
  legs,
  t,
}: {
...
export function fmtTransitDuration(
  seconds: number,
  t: (k: string, p?: Record<string, string | number>) => string
): string {
 ...
 */
export function TransitLegChips({
  legs,
  transfers,
  size = 'sm',
  t,
  ...
  const badgeFont =
    size === 'sm' ? 'calc(9.5px * var(--fs-scale-caption, 1))' : 'calc(10.5px * var(--fs-scale-caption, 1))';
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: size === 'sm' ? 4 : 5, flexWrap: 'wrap' }}>
      {shown.map((leg, i) => (
        <React.Fragment key={i}>
          {i > 0 && (
            <span className="text-content-faint" style={{ fontSize: size === 'sm' ? 9 : 10 }}>
              ›
            </span>
          )}
          {leg.mode === 'WALK' ? (
            <span
              className="text-content-faint"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 2, fontSize: badgeFont, fontWeight: 600 }}
            >
            ...
            <span
              style={{
                ...
                lineHeight: size === 'sm' ? '15px' : undefined,
                fontSize: badgeFont,
                fontWeight: 700,
                background: leg.line_color || 'var(--bg-tertiary)',
                color: leg.line_color ? leg.line_text_color || '#fff' : 'var(--text-primary)',
