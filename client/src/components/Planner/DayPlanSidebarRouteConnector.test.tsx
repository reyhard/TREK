...
describe('RouteConnector transit action', () => {
  ...
  it('opens only after clicking and runs the selected action once', async () => {
    ...
    render(<RouteConnector seg={seg} profile="walking" transitAction={transitAction(onSelect)} />);
  ...
  it('provides a mobile-sized touch target without enlarging the visible connector row', () => {
    render(<RouteConnector seg={seg} profile="walking" transitAction={transitAction()} />);
  ...
  it('closes when the user clicks outside', async () => {
    ...
    render(
      <div>
        ...
        <RouteConnector seg={seg} profile="walking" transitAction={transitAction()} />
      </div>
    );
  ...
  it('clamps using transform-independent menu dimensions while measuring the trigger bounds', async () => {
    ...
    const offsetHeightSpy = vi.spyOn(HTMLElement.prototype, 'offsetHeight', 'get').mockImplementation(function () {
      return this.getAttribute('role') === 'menu' ? 76 : 0;
    });
