...
export default function TripWarningsBanner({
  tripId,
  onOpenPluginTab,
}: {
  tripId: number;
  ...
  useEffect(() => {
    setNavSlot(document.getElementById('trek-nav-center-slot'));
  }, []);
  useEffect(() => {
    ...
    <>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: s.bg,
                  color: s.color,
                  cursor: onOpenPluginTab ? 'pointer' : 'default',
      ...
      {floating.length > 0 && (
        <div
          style={{
            position: 'absolute',
            bottom: 'calc(var(--bottom-nav-h, 0px) + 8px)',
            left: 0,
            right: 0,
            zIndex: 6,
            pointerEvents: 'none',
