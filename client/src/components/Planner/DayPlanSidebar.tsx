...
import {
  ...
  type DayMovementPlan,
  type TrackMovementPart,
} from '../../utils/dayMovementPlan';
import { getAccommodationAnchors, isDayInAccommodationRange } from '../../utils/dayOrder';
...
function useDayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  useEffect(() => {
    ...
    setRouteMetricStates(
      Object.fromEntries(
        plans.map(([dayId, plan]) => [dayId, { status: 'loading' as const, expected: plan.hasRoutedConnectors }])
      )
    );
        ...
        for (const part of resolved.parts) {
          ...
          switch (part.placement.kind) {
            case 'after-assignment':
              ui.assignmentConnectors[part.placement.assignmentId] = segment;
              break;
            case 'after-reservation':
              ui.reservationConnectors[part.placement.reservationId] = segment;
              break;
            case 'hotel-top':
              ui.hotelTop = { seg: segment, name: part.placement.name };
              break;
            case 'hotel-bottom':
              ui.hotelBottom = { seg: segment, name: part.placement.name };
              break;
          }
        }
    ...
    optimizeFromAccommodation,
    distanceUnit,
    routeMetricsGeneration,
  ]);

  const routeLegs = useMemo<Record<number, Record<number, RouteSegment>>>(
    () =>
      Object.fromEntries(
        Object.entries(movementUiByDay).map(([dayId, ui]) => [
          dayId,
          {
            ...
            ...Object.fromEntries(
              Object.entries(ui.reservationConnectors).map(([id, segment]) => [-Number(id), segment])
            ),
          },
        ])
      ),
    ...
    try {
      ...
      if (legResIds.length) {
        for (const ridStr of legResIds) {
          ...
          let parsed: any = {};
          try {
            parsed = safeParseMetadata(r as any);
          } catch {
            parsed = {};
          }
...
const DayPlanSidebar = React.memo(function DayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  const movementDistanceUnit =
    useSettingsStore((state) => state.settings.distance_unit) === 'imperial' ? 'imperial' : 'metric';
              ...
              {isExpanded && (
                ...
                >
                  {movementUiByDay[day.id]?.hotelTop && (
                    <HotelRouteConnector
                      seg={movementUiByDay[day.id]!.hotelTop!.seg}
                      name={movementUiByDay[day.id]!.hotelTop!.name}
                        ...
                        return (
                          <React.Fragment key={`transport-${res.id}-${legKey}-${day.id}`}>
                            ...
                            {!isTransitMergedItem(item) && movementUiByDay[day.id]?.reservationConnectors[res.id] && (
                              <RouteConnector
                                seg={movementUiByDay[day.id]!.reservationConnectors[res.id]}
                                profile={routeProfile}
                              />
                            )}
                          </React.Fragment>
                  ...
                  {movementUiByDay[day.id]?.hotelBottom && (
                    <HotelRouteConnector
                      seg={movementUiByDay[day.id]!.hotelBottom!.seg}
                      name={movementUiByDay[day.id]!.hotelBottom!.name}
                      profile={routeProfile}
                      placement="bottom"
                    />
                  )}
                      ...
                      {routeActive && (
                        <DayMovementTotalRow
                          ...
                          totalLabel={t('dayplan.movement.total', {
                            mode: t(
                              routeProfile === 'driving' ? 'dayplan.movement.driving' : 'dayplan.movement.walking'
                            ),
                          })}
