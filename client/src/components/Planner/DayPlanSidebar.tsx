...
function useDayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  useEffect(() => {
    ...
    const planDay = (dayId: number) => {
      ...
      const wayPts: { lat: number; lng: number; isPlace: boolean; time: string | null; carrierEdge?: 'departure' | 'arrival' | null; leg_transport_mode?: string | null; incoming_leg_transport_mode?: string | null; id?: number }[] = []
      for (const it of merged) {
        if (it.type === 'place') {
          const place = places.find(candidate => candidate.id === it.data.place?.id) ?? it.data.place
          if (place?.lat == null || place?.lng == null) continue
          ...
          if (from) wayPts.push({ lat: from.lat, lng: from.lng, isPlace: false, time: null, carrierEdge: isCarrierTransport(it.data) ? 'departure' : null })
          if (to) wayPts.push({ lat: to.lat, lng: to.lng, isPlace: false, time: null, carrierEdge: isCarrierTransport(it.data) ? 'arrival' : null })
        }
      }
...
const DayPlanSidebar = React.memo(function DayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  const can = useCanDo()
  const setAssignmentTime = useTripStore(s => s.setAssignmentTime)
  ...
  const selectTimelineDay = props.onTimelineSelectDay ?? (dayId => props.onSelectDay(dayId))

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '8px 12px', borderBottom: '1px solid var(--border-faint)', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        {mode === 'list' ? (
          ...
          <DayTimelinePlanner
            ...
            onAssignToDay={async (placeId, dayId) => {
              props.onAssignToDay(placeId, dayId)
              return undefined
            }}
            onSetAssignmentTime={(dayId, assignmentId, times) => setAssignmentTime(props.tripId, dayId, assignmentId, times)}
          ...
          <div style={{ display: 'grid', height: '100%', placeItems: 'center', padding: 24, textAlign: 'center' }}>
            <div style={{ display: 'grid', gap: 8, width: 'min(100%, 360px)' }}>
              ...
              {props.days.map(day => (
                <button key={day.id} type="button" onClick={() => selectTimelineDay(day.id)} className="text-content" style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '10px 12px', border: '1px solid var(--border-faint)', borderRadius: 8, background: 'var(--bg-surface)', cursor: 'pointer', textAlign: 'left' }}>
