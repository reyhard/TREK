import { Check, CheckSquare, ChevronDown, Layers, Tag, Tags } from 'lucide-react';
    ...
    <div className="col-filterbar">
      ...
      {showSelect && (
        <button
          ...
          className={`col-filter-btn col-filter-select${selectMode ? 'open' : ''}`}
      ...
      {showLabels && (labelOptions.length > 0 || canManageLabels) && (
        <div className="col-labelfilter">
          ...
          {canManageLabels && (
            <button
              type="button"
              className="col-labelchip col-labelchip-manage"
              onClick={onManageLabels}
              title={t('collections.labels.manage')}
            >
              <Tags size={13} />
              <span className="col-filter-lbl">
                {labelOptions.length ? t('collections.labels.manage') : t('collections.labels.add')}
              </span>
            </button>
          )}
        </div>
