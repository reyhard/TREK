...
describe('MapViewGL', () => {
  it.each([
    ...
    'FE-COMP-MAPVIEWGL-017: %s keeps stored GPX geometry separate from connector routes',
    async (_label, provider) => {
      ...
      expect(gpxData.features[0].geometry.coordinates).toEqual(track.map(([lat, lng]) => [lng, lat]));
    }
  );
  ...
  // FE-COMP-MAPVIEWGL-014: repositioning — only the active place is made draggable and drag events fire
  it.each([
    ['mapbox-gl', 'mapbox-gl'],
    ['maplibre-gl', 'maplibre-gl'],
  ] as const)(
    'FE-COMP-MAPVIEWGL-014: %s makes only the active place draggable and emits drag coordinates',
    async (_label, provider) => {
      glMap.on.mockImplementation((event: string, handlerOrLayer: unknown) => {
        if (event === 'load' && typeof handlerOrLayer === 'function') (handlerOrLayer as () => void)();
        return glMap;
  ...
  it('FE-COMP-MAPVIEWGL-015: reposition marker is excluded from cluster source data', async () => {
    const source = { setData: vi.fn() };
    glMap.getSource.mockImplementation((id: string) => {
      if (id === 'trip-place-clusters') return source;
      return null;
    });
    glMap.querySourceFeatures.mockReturnValue([]);
    glMap.on.mockImplementation((event: string, handlerOrLayer: unknown) => {
    ...
    render(<MapViewGL places={[place]} selectedPlaceId={9} repositionPlaceId={9} canRepositionPlaces />);
    await act(async () => {
