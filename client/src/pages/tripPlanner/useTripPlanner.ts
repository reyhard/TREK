...
export function useTripPlanner() {
  ...
  const handlePlaceRepositionEnd = useCallback((placeId: number, coordinates: { lat: number; lng: number }) => {
    if (repositionPlaceId !== placeId) return false
  ...
  const savePlaceReposition = useCallback(async (coordinates?: { lat: number; lng: number }) => {
    const pending = coordinates ?? repositionPending
    ...
    try {
      ...
      setRepositionPending(null)
      repositionOriginalRef.current = null
      toast.error(err instanceof Error ? err.message : t('inspector.moveFailed'))
    } finally {
  ...
  useEffect(() => {
    if (repositionPlaceId != null && repositionPlaceId !== selectedPlaceId && !repositionSaving) {
      setRepositionPlaceId(null)
      setRepositionPending(null)
      repositionOriginalRef.current = null
    }
  }, [repositionPlaceId, repositionSaving, selectedPlaceId])
  ...
  const mapPlaces = useMemo(() => {
    ...
    const plannedIds = placesFilter === 'planned' || placesFilter === 'unplanned'
      ? new Set((selectedDayId
        ...
        : Object.values(assignments).flatMap(dayAssignments => dayAssignments)
