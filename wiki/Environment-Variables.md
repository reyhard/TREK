...
| `OAUTH_HTTP_REDIRECT_HOSTS` | Comma-separated list of LAN hostnames allowed for HTTP OAuth redirect URIs (default: only `localhost`/`127.0.0.1`/`[::1]`). HTTPS redirect URIs always allow the `APP_URL` hostname and do not need this variable. | `localhost`, `127.0.0.1`, `[::1]` |
...
| `TREK_PLUGIN_LOG_BURST`           | Per-plugin burst allowance for log lines — how many log lines a plugin may emit back-to-back before the per-second rate limit applies. | `50`                                                                                |
| `TREK_PLUGIN_LOG_PER_SEC`         | Sustained rate limit (log lines **per second**) for a plugin's log output once its burst allowance is spent. | `10`                                                                                |
