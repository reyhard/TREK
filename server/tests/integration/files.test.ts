...
const testFilesDir = vi.hoisted(() => {
  ...
  const os = require('os');
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'trek-files-test-'));
  return dir;
});
...
beforeAll(async () => {
  ...
  // The v4 storage registry owns the upload root. Override its built-in
  // uploads-local backend in the test database instead of using the removed
  // TREK_FILES_DIR setting.
  testDb
    .prepare("INSERT OR REPLACE INTO app_settings (key, value) VALUES ('storage.backends', ?)")
    .run(JSON.stringify([{ name: 'uploads-local', type: 'local', options: { root: testFilesDir } }]));
...
describe('Upload file', () => {
  ...
  it('FILE-P01 — stored filename is a bare uuid name and the bytes land in uploads/files', async () => {
    ...
    expect(fs.existsSync(path.join(filesDir, res.body.file.filename))).toBe(true);
...
describe('File download', () => {
  ...
  it('FILE-P06 — a DB row whose bytes are gone answers 404 File not found', async () => {
    ...
    const upload = await uploadFile(trip.id, user.id, FIXTURE_PDF);
    const fileId = upload.body.file.id;
    fs.rmSync(path.join(filesDir, upload.body.file.filename), { force: true });
