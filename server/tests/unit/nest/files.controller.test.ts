...
describe('FilesController (parity with the legacy /api/trips/:tripId/files route)', () => {
  ...
  describe('POST / (upload)', () => {
    ...
    it('caps non-video by extension and accepts large video; cleans up on rejection (#823)', () => {
      ...
      expect(thrown(() => new FilesController(fsvc()).upload(user, '5', big, {}))).toEqual({
        status: 400,
      ...
      expect(
        new FilesController(fsvc({ createFile, broadcast: vi.fn() } as Partial<FilesService>)).upload(
          user,
          '5',
          vid,
          {},
      ...
      expect(
        thrown(() =>
          new FilesController(fsvc({ can: vi.fn().mockReturnValue(false) })).upload(user, '5', withPath, {}),
        ),
      ).toEqual({ status: 403, body: { error: 'No permission to upload files' } });
    });
  });
