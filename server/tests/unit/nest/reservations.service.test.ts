...
describe('ReservationsService — referenced ids stay inside the trip', () => {
  ...
  it('updates only requested transit endpoints and preserves the reservation row', () => {
    const { trip } = ownerTrip();
    const { reservation } = svc.create(trip.id, {
      title: 'A to B',
      type: 'transit',
      endpoints: [
        { role: 'from', name: 'A', code: 'A', lat: 34.9, lng: 135.7, timezone: null, local_time: null, local_date: null },
        { role: 'to', name: 'B', code: 'B', lat: 35, lng: 135.8, timezone: null, local_time: null, local_date: null },
      ],
    });
    const before = testDb.prepare('SELECT * FROM reservations WHERE id = ?').get(reservation.id);

    const updated = svc.updateTransitRouteEndpoints(reservation.id, trip.id, {
      from: { name: 'New A', lat: 34.91, lng: 135.71 },
    });

    expect(updated.endpoints).toEqual(expect.arrayContaining([
      expect.objectContaining({ role: 'from', name: 'New A', lat: 34.91, lng: 135.71 }),
      expect.objectContaining({ role: 'to', name: 'B', lat: 35, lng: 135.8 }),
    ]));
    expect(testDb.prepare('SELECT * FROM reservations WHERE id = ?').get(reservation.id)).toEqual(before);
  });

  it('rolls back an endpoint update when the requested route structure is invalid', () => {
    const { trip } = ownerTrip();
    const { reservation } = svc.create(trip.id, {
      title: 'A to B',
      type: 'transit',
      endpoints: [{ role: 'from', name: 'A', code: 'A', lat: 34.9, lng: 135.7, timezone: null, local_time: null, local_date: null }],
    });
    const before = testDb.prepare('SELECT name, lat, lng FROM reservation_endpoints WHERE reservation_id = ?').all(reservation.id);

    expect(() => svc.updateTransitRouteEndpoints(reservation.id, trip.id, {
      from: { name: 'Changed A', lat: 34.91, lng: 135.71 },
      to: { name: 'Missing B', lat: 35, lng: 135.8 },
    })).toThrowError(expect.objectContaining({ code: 'ENDPOINT_STRUCTURE_INVALID' }));
    expect(testDb.prepare('SELECT name, lat, lng FROM reservation_endpoints WHERE reservation_id = ?').all(reservation.id)).toEqual(before);
  });
});
