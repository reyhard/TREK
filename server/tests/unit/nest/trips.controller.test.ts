...
describe('TripsController (parity with the legacy /api/trips route)', () => {
  ...
  describe('PUT /:id', () => {
    it('404 when no access; 403 on archive without trip_archive', async () => {
      ...
      expect(await thrownAsync(() => new TripsController(s).update(user, '9', { is_archived: 1 }, req))).toEqual({
        status: 403,
    ...
    it('403 on cover_image without trip_cover_upload', async () => {
      ...
      expect(await thrownAsync(() => new TripsController(s).update(user, '9', { cover_image: '/x.jpg' }, req))).toEqual(
        { status: 403, body: { error: 'No permission to change cover image' } },
      );
    });

    it('403 on an edit field without trip_edit', async () => {
      const s = svc({ can: vi.fn().mockImplementation((a: string) => a !== 'trip_edit') });
      expect(await thrownAsync(() => new TripsController(s).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 403,
    ...
    it('maps a NotFoundError to 404 and a ValidationError to 400', async () => {
      ...
      expect(await thrownAsync(() => new TripsController(nf).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 404,
        body: { error: 'gone' },
      });
      const ve = svc({
      ...
      expect(await thrownAsync(() => new TripsController(ve).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 400,
        body: { error: 'bad' },
      });
    });
    ...
    it('#1277: internalises an Unsplash cover hot-link into uploads/covers before saving', async () => {
      ...
      await new TripsController(s).update(
        user,
        '9',
        { cover_image: 'https://images.unsplash.com/photo-123?w=1080' },
        req,
      );
  ...
  describe('POST /:id/transfer (#973)', () => {
    it('404 without trip access', () => {
      const s = svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) });
      expect(thrown(() => new TripsController(s).transferOwnership(user, '9', 2, req))).toEqual({
        status: 404,
        body: { error: 'Trip not found' },
      });
    });
    ...
    it('400 when newOwnerId is not a number', () => {
      const s = svc();
      expect(
        thrown(() => new TripsController(s).transferOwnership(user, '9', 'nope' as unknown as number, req)),
      ).toEqual({ status: 400, body: { error: 'newOwnerId is required' } });
    });
    ...
    it('maps NotFoundError to 404 and ValidationError to 400', () => {
      ...
      expect(thrown(() => new TripsController(nf).transferOwnership(user, '9', 2, req))).toEqual({
        status: 404,
      ...
      expect(thrown(() => new TripsController(ve).transferOwnership(user, '9', 2, req))).toEqual({
        status: 400,
  ...
  describe('guests (#1362)', () => {
    it('404 without access, 403 for a non-owner, 400 without a name; else creates', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).createGuest(
            user,
            '9',
            'Anna',
          ),
      ...
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue({ user_id: 5 }) })).createGuest(
            user,
            '9',
            'Anna',
          ),
        ),
      ).toEqual({ status: 403, body: { error: 'Only the owner can manage guests' } });
      expect(thrown(() => new TripsController(svc()).createGuest(user, '9', '  '))).toEqual({
        status: 400,
      ...
      expect(new TripsController(s).createGuest(user, '9', 'Anna')).toEqual({
    ...
    it('rename: 403 non-owner, 404 when the guest is missing, else success', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue({ user_id: 5 }) })).renameGuest(
            user,
            '9',
            '7',
            'Bob',
      ...
      expect(thrown(() => new TripsController(miss).renameGuest(user, '9', '7', 'Bob'))).toEqual({
        status: 404,
    ...
    it('delete: 403 non-owner, 404 when the guest is missing, else success', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue({ user_id: 5 }) })).deleteGuest(
            user,
            '9',
            '7',
          ),
      ...
      expect(thrown(() => new TripsController(miss).deleteGuest(user, '9', '7'))).toEqual({
        status: 404,
        body: { error: 'Guest not found' },
      });
    ...
    it('maps a ValidationError from createGuest to 400', () => {
      ...
      expect(thrown(() => new TripsController(ve).createGuest(user, '9', 'x'.repeat(60)))).toEqual({
        status: 400,
