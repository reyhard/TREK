...
import { PlaceCreateDto, PlaceUpdateDto } from '../../../src/nest/places/places.dto';
...
describe('PlacesController (parity with the legacy /api/trips/:tripId/places route)', () => {
  ...
  describe('POST / (create)', () => {
    it.each([4, 60.5, 1441])('rejects invalid recommended duration %s in the DTO schema', (duration) => {
      expect(PlaceCreateDto.schema.safeParse({ name: 'Spot', duration_minutes: duration }).success).toBe(false);
    });
  ...
  describe('PUT /:id (update)', () => {
    it.each([4, 60.5, 1441])('rejects invalid recommended duration %s in the DTO schema', (duration) => {
      expect(PlaceUpdateDto.schema.safeParse({ duration_minutes: duration }).success).toBe(false);
    });
  });
