...
describe('SettingsController', () => {
  ...
  it('PUT / 400 without a key', () => {
    expect(thrown(() => new SettingsController(svc()).upsert(user, {}))).toEqual({
      status: 400,
  ...
  it('POST /bulk 400 without an object, 500 on a write error, else returns the count', () => {
    expect(thrown(() => new SettingsController(svc()).bulk(user, {}))).toEqual({
      status: 400,
      body: { error: 'Settings object is required' },
    });
    ...
    expect(
      new SettingsController(svc({ bulkUpsertSettings: vi.fn().mockReturnValue(3) } as Partial<SettingsService>)).bulk(
        user,
