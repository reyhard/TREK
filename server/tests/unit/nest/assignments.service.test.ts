...
describe('updateTime', () => {
  ...
  it('ASG-SVC-030: an omitted timing request is a no-op and preserves order and values', () => {
    const { day, place } = fixture();
    const a = createDayAssignment(testDb, day.id, place.id, { order_index: 0 });
    const b = createDayAssignment(testDb, day.id, place.id, { order_index: 1 });
    testDb.prepare('UPDATE day_assignments SET assignment_time = ?, assignment_end_time = ? WHERE id = ?').run('10:00', '11:00', a.id);
    testDb.prepare('UPDATE day_assignments SET assignment_time = ?, assignment_end_time = ? WHERE id = ?').run('09:00', '10:00', b.id);

    const before = testDb.prepare('SELECT assignment_time, assignment_end_time, order_index FROM day_assignments WHERE id = ?').get(a.id);
    expect(testDb.prepare('SELECT id FROM day_assignments WHERE day_id = ? ORDER BY order_index').all(day.id)).toEqual([{ id: a.id }, { id: b.id }]);
    const updated = svc.updateTime(a.id, undefined, undefined);
    const after = testDb.prepare('SELECT assignment_time, assignment_end_time, order_index FROM day_assignments WHERE id = ?').get(a.id);

    expect(after).toEqual(before);
    expect(updated).toMatchObject({ assignment_time: '10:00', assignment_end_time: '11:00' });
    expect(testDb.prepare('SELECT id FROM day_assignments WHERE day_id = ? ORDER BY order_index').all(day.id)).toEqual([{ id: a.id }, { id: b.id }]);
  });

  it('ASG-SVC-031: an explicit null shadows inherited place times for only that assignment', () => {
    const { day, place } = fixture();
    testDb.prepare('UPDATE places SET place_time = ?, end_time = ? WHERE id = ?').run('09:00', '10:00', place.id);
    const cleared = createDayAssignment(testDb, day.id, place.id, { order_index: 0 });
    const inherited = createDayAssignment(testDb, day.id, place.id, { order_index: 1 });

    const updated = svc.updateTime(cleared.id, null, null);
    expect(updated).toMatchObject({
      assignment_time: null,
      assignment_end_time: null,
      place: { place_time: null, end_time: null },
    });
    expect(testDb.prepare('SELECT assignment_time, assignment_end_time FROM day_assignments WHERE id = ?').get(cleared.id)).toEqual({
      assignment_time: '',
      assignment_end_time: '',
    });
    expect(svc.listDayAssignments(day.id).find((assignment) => assignment.id === inherited.id)).toMatchObject({
      place: { place_time: '09:00', end_time: '10:00' },
    });
  });
  ...
  it('ASG-SVC-027: an empty-string time clears the override like null (and skips the auto-sort)', () => {
    ...
    expect(testDb.prepare('SELECT assignment_time FROM day_assignments WHERE id = ?').get(b.id)).toEqual({ assignment_time: '' });
    const order = (id: number) => (testDb.prepare('SELECT order_index FROM day_assignments WHERE id = ?').get(id) as { order_index: number }).order_index;
    expect(order(b.id)).toBe(0);
    expect(order(a.id)).toBe(1);
  });

  it('ASG-SVC-024: a late valid time sorts after an earlier timed assignment', () => {
    const { day, place } = fixture();
    const a = createDayAssignment(testDb, day.id, place.id, { order_index: 0 });
    const b = createDayAssignment(testDb, day.id, place.id, { order_index: 1 });
    svc.updateTime(a.id, '23:59', null);
