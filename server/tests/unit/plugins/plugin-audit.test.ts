...
describe('appendAudit hash chain', () => {
  ...
  it('chains each entry off the previous hash (per plugin)', () => {
    ...
    const rows = db.prepare('SELECT prev_hash, hash FROM plugin_capability_audit ORDER BY id').all() as Array<{
  ...
  it('keeps separate chains per plugin', () => {
    ...
    const a = db.prepare("SELECT prev_hash FROM plugin_capability_audit WHERE plugin_id='b'").get() as {
      prev_hash: string | null;
    };
  ...
  it("readAuditForUser returns one user's actions across ALL plugins, newest first, with the plugin name", () => {
    ...
    expect(
      mine.some(
        (r) =>
          r.plugin_id === 'koffi' && r.method === 'trips.getById' && (r as { resource?: string }).resource === 'trip:2',
      ),
    ).toBe(false); // never another user's
  });
  ...
  it('pruneAudit keeps only the newest N rows per plugin, leaving the retained window chain-consistent', () => {
    for (let i = 0; i < 50; i++)
      appendAudit(db, { pluginId: 'p', actingUserId: 1, method: 'trips.getById', resource: `trip:${i}`, code: 'ok' });
    appendAudit(db, { pluginId: 'other', actingUserId: 1, method: 'trips.getById', resource: 'trip:x', code: 'ok' });
    pruneAudit(db, 'p', 10);
    const rows = db
      .prepare("SELECT resource, prev_hash, hash FROM plugin_capability_audit WHERE plugin_id = 'p' ORDER BY id ASC")
      .all() as Array<{ resource: string; prev_hash: string | null; hash: string }>;
    ...
    expect(db.prepare("SELECT COUNT(*) c FROM plugin_capability_audit WHERE plugin_id='p'").get()).toMatchObject({
      c: 11,
    });
    // pruning one plugin never touches another's rows
    expect(db.prepare("SELECT COUNT(*) c FROM plugin_capability_audit WHERE plugin_id='other'").get()).toMatchObject({
      c: 1,
    });
