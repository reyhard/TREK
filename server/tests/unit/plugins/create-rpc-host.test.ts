...
vi.mock('../../../src/db/database', () => {
  ...
  return {
    ...
    canAccessTrip: (tripId: number, userId: number) =>
      tripId === 1 && (userId === 5 || userId === 6 || userId === 77) ? { id: 1, user_id: 5 } : undefined,
  };
});
...
vi.mock('../../../src/services/dayService', () => ({
  ...
  deleteDay: vi.fn(),
  listDays: vi.fn((tid: number) => ({
    days: [{ id: 3, trip_id: Number(tid), day_number: 1, assignments: [], notes_items: [] }],
  })),
  ...
  createAccommodation: vi.fn((tid: number, data: Record<string, unknown>) => ({
    id: 60,
    trip_id: Number(tid),
    ...data,
  })),
  getAccommodation: vi.fn((id: number) =>
    Number(id) === 404 ? undefined : { id: Number(id), place_id: 7, start_day_id: 3, end_day_id: 4 },
  ),
  updateAccommodation: vi.fn((id: number, _existing: unknown, fields: Record<string, unknown>) => ({
    id: Number(id),
    ...fields,
  })),
  deleteAccommodation: vi.fn((id: number) =>
    Number(id) === 61
      ? { linkedReservationId: 40, deletedBudgetItemId: 9 }
      : { linkedReservationId: null, deletedBudgetItemId: null },
  ),
}));
vi.mock('../../../src/services/assignmentService', () => ({
  createAssignment: vi.fn((dayId: number, placeId: number, notes: string | null) => ({
    id: 30,
    day_id: dayId,
    place_id: placeId,
    notes,
...
vi.mock('../../../src/services/packingService', () => ({
  ...
  createItem: vi.fn(
    (tid: number, input: { name: string; visibility?: string; recipient_ids?: number[] }, ownerId?: number) => {
      if (input.visibility === 'personal')
        return { id: 70, trip_id: Number(tid), name: input.name, is_private: 1, owner_id: ownerId, recipients: [] };
      if (input.visibility === 'shared')
        return {
          ...
          is_private: 1,
          owner_id: ownerId,
          recipients: (input.recipient_ids || []).map((id) => ({ user_id: id })),
        };
  ...
  updateItem: vi.fn((tid: number, id: string, input: { is_private?: boolean }) =>
    Number(id) === 99
      ? { conflict: true, server: { id: 99 } }
      : { id: Number(id), trip_id: Number(tid), is_private: input.is_private ? 1 : 0, owner_id: 5 },
  ),
  // The raw deleted row (owner-only for a private item, no recipients — #858).
  deleteItem: vi.fn((_tid: number, id: string) =>
    Number(id) === 404
      ? null
      : Number(id) === 71
        ? { id: 71, is_private: 1, owner_id: 5 }
        : { id: Number(id), is_private: 0 },
  ),
  ...
  updateBag: vi.fn((_tid: number, bagId: string) =>
    Number(bagId) === 404 ? null : { id: Number(bagId), name: 'Renamed' },
  ),
  deleteBag: vi.fn((_tid: number, bagId: string) => Number(bagId) !== 404),
  setBagMembers: vi.fn((_tid: number, bagId: string, userIds: number[]) =>
    Number(bagId) === 404 ? null : userIds.map((u) => ({ user_id: u })),
  ),
}));
vi.mock('../../../src/services/conflictResult', () => ({
  isUpdateConflict: (r: unknown) => !!(r as { conflict?: boolean })?.conflict,
}));
vi.mock('../../../src/services/weatherService', () => ({
  getWeather: vi.fn(async (lat: string, lng: string) => ({ lat, lng, temp: 20 })),
}));
...
vi.mock('../../../src/services/tagService', () => ({
  ...
  getTagByIdAndUser: vi.fn((tagId: number, _uid: number) =>
    Number(tagId) === 404 ? undefined : { id: Number(tagId) },
  ),
...
vi.mock('../../../src/services/fileService', () => ({
  ...
  createFile: vi.fn(
    (tid: number, file: { filename: string; originalname: string; size: number }, uploadedBy: number) => ({
      id: 130,
      trip_id: Number(tid),
      ...file,
  ...
  resolveFilePath: vi.fn((filename: string) => ({
    resolved: `${testFilesDir}/${filename}`,
    safe: filename !== 'evil',
  })),
...
vi.mock('../../../src/services/collabService', () => ({
  ...
  listMessages: vi.fn((tid: number, before?: number) => [
    { id: 3, trip_id: Number(tid), text: 'hi', _before: before ?? null },
  ]),
  createNote: vi.fn((tid: number, uid: number, data: { title: string }) => ({
    id: 140,
    trip_id: Number(tid),
    created_by: uid,
    title: data.title,
  })),
  createPoll: vi.fn((tid: number, _uid: number, data: { question: string }) => ({
    id: 141,
    trip_id: Number(tid),
  ...
  createMessage: vi.fn((tid: number, uid: number, text: string) =>
    text === 'toolong'
      ...
      : { message: { id: 142, trip_id: Number(tid), user_id: uid, text } },
  ),
}));
...
vi.mock('../../../src/nest/llm-parse/llm-config.resolver', () => ({
  resolveLlmConfig: vi.fn((uid: number) =>
...
vi.mock('../../../src/nest/plugins/plugins.service', () => ({
  readUserSettingDecrypted: vi.fn((_pid: string, uid: number, key: string) =>
    uid === 5 && key === 'apiKey' ? 'k-5' : undefined,
  ),
}));
vi.mock('../../../src/nest/plugins/plugin-oauth.service', () => ({
  PluginOAuthService: class {
...
vi.mock('../../../src/nest/reservations/reservations.service', () => ({
  ReservationsService: class {
    ...
    getReservation(id: string) {
      return id === '404' ? undefined : { id: Number(id), title: 'Old', type: 'flight' };
    }
    update(id: string, tid: string, input: Record<string, unknown>) {
      return {
        reservation: { id: Number(id), trip_id: Number(tid), ...input },
        accommodationChanged: input.title === 'New',
      };
    }
    remove(id: string) {
      if (id === '404') return { deleted: undefined, accommodationDeleted: false, deletedBudgetItemId: null };
      return {
        deleted: { id: Number(id), title: 'Gone', type: 'hotel', accommodation_id: 7 },
        accommodationDeleted: true,
        deletedBudgetItemId: 9,
      };
    }
    list(tid: string) {
      return [{ id: 1, trip_id: Number(tid), title: 'Flight' }];
    }
    syncBudgetOnCreate() {}
    syncBudgetOnUpdate() {}
    notifyBookingChange() {}
...
vi.mock('../../../src/services/journeyService', () => ({
  ...
  listEntries: vi.fn((journeyId: number, uid: number) =>
    journeyId === 88 ? null : [{ id: 10, journey_id: journeyId, author_id: uid }],
  ),
  // journeyId 99 = not editable by the acting user (canEdit inside returns null/false)
  createEntry: vi.fn((journeyId: number, uid: number, data: unknown) =>
    journeyId === 99 ? null : { id: 120, journey_id: journeyId, created_by: uid, ...(data as object) },
  ),
  updateEntry: vi.fn((entryId: number, _uid: number, data: unknown) =>
    entryId === 99 ? null : { id: entryId, ...(data as object) },
  ),
...
vi.mock('../../../src/services/collectionsService', () => {
  ...
  return {
    ...
    updateCollection: vi.fn((_uid: number, id: number, body: unknown) => {
      if (id === 99) httpError(403, 'read-only');
      if (id === 404) httpError(404, 'Collection not found');
    ...
    deletePlace: vi.fn((_uid: number, placeId: number) => {
      if (placeId === 404) httpError(404, 'Collection not found');
    }),
  };
});
...
describe('create-rpc-host wiring', () => {
  ...
  it('a granted db:own call runs against the plugin db, and a trip broadcast is namespaced', async () => {
    ...
    const migrated = await host.dispatch({
      ...
      id: '1',
      method: 'db.migrate',
      params: { id: '001', sql: 'CREATE TABLE t (v TEXT)' },
    });
    expect(migrated.ok).toBe(true);

    // acting user 5 is a member of trip 1 (mocked canAccessTrip) → broadcast allowed + namespaced
    await host.dispatch(
      { k: 'req', id: '2', method: 'ws.broadcastToTrip', params: { tripId: 1, event: 'ping', data: { a: 1 } } },
      5,
    );
    ...
    await bcastUser.dispatch(
      { k: 'req', id: '3', method: 'ws.broadcastToUser', params: { userId: 5, event: 'hi', data: {} } },
      5,
    );
  ...
  it('db.tx dispatches an atomic batch through to the plugin db under db:own', async () => {
    ...
    await host.dispatch({
      ...
      id: '1',
      method: 'db.migrate',
      params: { id: '001', sql: 'CREATE TABLE kv (k TEXT, n INTEGER)' },
    });
...
describe('create-rpc-host — planner write + metadata deps', () => {
  ...
  const call = async (
    ...
    uid = 5,
  ): Promise<any> => h.dispatch({ k: 'req', id: 'x', method, params }, uid);
  beforeEach(() => {
  ...
  it('places.create/update/delete delegate + broadcast; a missing place is RESOURCE_FORBIDDEN', async () => {
    ...
    expect((await call(h, 'places.update', { tripId: 1, placeId: 99, input: {} })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  ...
  it('days + itinerary delegate; a day/place/assignment outside the trip is refused', async () => {
    ...
    expect((await call(h, 'itinerary.assign', { tripId: 1, dayId: 99, placeId: 7 })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    expect((await call(h, 'itinerary.assign', { tripId: 1, dayId: 3, placeId: 99 })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    expect((await call(h, 'itinerary.unassign', { tripId: 1, assignmentId: 30 })).ok).toBe(true);
    expect((await call(h, 'itinerary.unassign', { tripId: 1, assignmentId: 99 })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('trips.update: archive/cover need their own permission; service errors map to RPC codes', async () => {
    ...
    expect((await call(h, 'trips.update', { tripId: 1, input: { is_archived: 1 } })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    checkPermission.mockImplementation((action: string) => action !== 'trip_cover_upload');
    expect((await call(h, 'trips.update', { tripId: 1, input: { cover_image: '/x.jpg' } })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    checkPermission.mockReturnValue(true);
    expect((await call(h, 'trips.update', { tripId: 1, input: { title: 'boom' } })).error.code).toBe('BAD_PARAMS');
    expect((await call(h, 'trips.update', { tripId: 1, input: { title: 'gone' } })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  ...
  it('metadata: round-trips and enforces the key/value/access limits', async () => {
    ...
    expect((await call(h, 'meta.delete', { entityType: 'trip', entityId: 1, key: 'k' })).result).toEqual({
      deleted: true,
    });
    expect(
      (await call(h, 'meta.set', { entityType: 'trip', entityId: 1, key: 'x'.repeat(300), value: 1 })).error.code,
    ).toBe('BAD_PARAMS');
    expect(
      (await call(h, 'meta.set', { entityType: 'trip', entityId: 1, key: 'big', value: 'y'.repeat(70000) })).error.code,
    ).toBe('BAD_PARAMS');
    expect((await call(h, 'meta.set', { entityType: 'trip', entityId: 2, key: 'k', value: 1 })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('costs deps: update of a missing item is RESOURCE_FORBIDDEN', async () => {
    const h = host('db:write:costs');
    expect((await call(h, 'costs.update', { tripId: 1, itemId: 404, input: { name: 'X' } })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('packing/files read deps delegate to their services (trash excluded for files)', async () => {
    ...
    expect((await call(h, 'packing.list', { tripId: 1 })).result).toEqual([
      { id: 1, trip_id: 1, name: 'Socks', _uid: 5 },
    ]);
...
describe('create-rpc-host — reservations, day notes, cross-trip + addon reads (Waves 1-5)', () => {
  ...
  it('reservations create/update/delete run the real side-effect wiring; a missing one is refused', async () => {
    ...
    expect((await call(h, 'reservations.update', { tripId: 1, reservationId: 40, input: { title: 'New' } })).ok).toBe(
      true,
    );
    expect(
      (await call(h, 'reservations.update', { tripId: 1, reservationId: 404, input: { title: 'X' } })).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
    expect((await call(h, 'reservations.delete', { tripId: 1, reservationId: 40 })).ok).toBe(true);
    expect((await call(h, 'reservations.delete', { tripId: 1, reservationId: 404 })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('day notes create/update/delete run the wiring; a day/note outside the trip is refused', async () => {
    ...
    expect((await call(h, 'daynotes.create', { tripId: 1, dayId: 88, input: { text: 'x' } })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    expect((await call(h, 'daynotes.update', { tripId: 1, dayId: 3, noteId: 5, input: { text: 'y' } })).ok).toBe(true);
    expect((await call(h, 'daynotes.update', { tripId: 1, dayId: 3, noteId: 99, input: {} })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    expect((await call(h, 'daynotes.delete', { tripId: 1, dayId: 3, noteId: 5 })).ok).toBe(true);
    expect((await call(h, 'daynotes.delete', { tripId: 1, dayId: 3, noteId: 99 })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('wave-13 wiring: collab reads, journal entries, atlas bucket, file content, trip create, rates delegate correctly', async () => {
    ...
    expect((await call(collab, 'collab.listNotes', { tripId: 1 })).result).toEqual([
      { id: 1, trip_id: 1, title: 'Note' },
    ]);
    expect((await call(collab, 'collab.listMessages', { tripId: 1, before: 9 })).result).toEqual([
      { id: 3, trip_id: 1, text: 'hi', _before: 9 },
    ]);
    ...
    expect((await call(journal, 'journal.getEntries', { journeyId: 7 })).result).toEqual([
      { id: 10, journey_id: 7, author_id: 5 },
    ]);
    ...
    expect((await call(create, 'trips.create', { input: { title: 'Japan' } })).result).toMatchObject({
      id: 99,
      user_id: 5,
      title: 'Japan',
    });
  ...
  it('accommodations create validates refs, creates via dayService and emits the cascade broadcasts', async () => {
    ...
    const good = await call(h, 'accommodations.create', {
      tripId: 1,
      input: { place_id: 7, start_day_id: 3, end_day_id: 4, check_in: '15:00' },
    });
    ...
    expect(
      (await call(h, 'accommodations.create', { tripId: 1, input: { place_id: 999, start_day_id: 3, end_day_id: 4 } }))
        .error.code,
    ).toBe('RESOURCE_FORBIDDEN');
    expect(
      (await call(h, 'accommodations.create', { tripId: 1, input: { place_id: 7, start_day_id: 88, end_day_id: 4 } }))
        .error.code,
    ).toBe('RESOURCE_FORBIDDEN');
  });
  ...
  it('accommodations update/delete scope the row to the trip; delete cascades reservation + budget broadcasts', async () => {
    const h = host('db:write:accommodations');
    expect(
      (await call(h, 'accommodations.update', { tripId: 1, accommodationId: 60, input: { notes: 'late checkout' } }))
        .ok,
    ).toBe(true);
    expect(broadcast).toHaveBeenCalledWith(1, 'accommodation:updated', expect.anything());
    expect((await call(h, 'accommodations.update', { tripId: 1, accommodationId: 404, input: {} })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    ...
    expect((await call(h, 'accommodations.delete', { tripId: 1, accommodationId: 404 })).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
...
describe('create-rpc-host — packing write with #858 privacy-scoped broadcasts', () => {
  ...
  it('create: Common -> whole room; Personal -> owner-only; Shared -> owner + recipients', async () => {
    expect((await call('packing.create', { tripId: 1, input: { name: 'Common', visibility: 'common' } })).ok).toBe(
      true,
    );
    expect(fanout('packing:created')).toEqual([undefined]); // whole room

    broadcast.mockClear();
    expect((await call('packing.create', { tripId: 1, input: { name: 'Mine', visibility: 'personal' } })).ok).toBe(
      true,
    );
    ...
    broadcast.mockClear();
    expect(
      (await call('packing.create', { tripId: 1, input: { name: 'Ours', visibility: 'shared', recipient_ids: [6] } }))
        .ok,
    ).toBe(true);
...
describe('create-rpc-host — Wave 1 wiring (weather/categories/tags/todos/roster/bags)', () => {
  ...
  it("tags: list/create/update/delete are the acting user's; a missing tag is RESOURCE_FORBIDDEN", async () => {
    ...
    expect(
      ((await call(h, 'tags.update', { tagId: 404, input: {} }, 5)) as { error: { code: string } }).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
    expect(((await call(h, 'tags.delete', { tagId: 404 }, 5)) as { error: { code: string } }).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('todos list/create/update/delete run the wiring; a missing one is RESOURCE_FORBIDDEN', async () => {
    ...
    expect(
      ((await call(h, 'todos.update', { tripId: 1, todoId: 404, input: {} }, 5)) as { error: { code: string } }).error
        .code,
    ).toBe('RESOURCE_FORBIDDEN');
    expect(
      ((await call(h, 'todos.delete', { tripId: 1, todoId: 404 }, 5)) as { error: { code: string } }).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
  });
  ...
  it('packing bags list/create/update/delete/setMembers run the wiring', async () => {
    ...
    expect(
      ((await call(h, 'packing.deleteBag', { tripId: 1, bagId: 404 }, 5)) as { error: { code: string } }).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
  });
});
...
describe('create-rpc-host — Wave 2 wiring (atlas/vacay/journal/collections writes)', () => {
  ...
  it('atlas writes delegate uid-scoped; disabled addon refused; missing bucket item forbidden', async () => {
    ...
    expect(((await call(h, 'atlas.deleteBucketItem', { itemId: 404 })) as { error: { code: string } }).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
    isAddonEnabled.mockReturnValue(false);
    expect(((await call(h, 'atlas.markCountry', { code: 'JP' })) as { error: { code: string } }).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('journal writes map an uneditable journey/entry to RESOURCE_FORBIDDEN', async () => {
    ...
    expect(
      (
        (await call(h, 'journal.createEntry', { journeyId: 99, input: { entry_date: '2026-08-01' } })) as {
          error: { code: string };
        }
      ).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
    ...
    expect(
      ((await call(h, 'journal.updateEntry', { entryId: 99, input: {} })) as { error: { code: string } }).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
    expect((await call(h, 'journal.deleteEntry', { entryId: 120 })).ok).toBe(true);
    expect(((await call(h, 'journal.deleteEntry', { entryId: 99 })) as { error: { code: string } }).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  });
  ...
  it('collections writes map the service 403/404 to RESOURCE_FORBIDDEN', async () => {
    ...
    expect(
      ((await call(h, 'collections.update', { id: 99, input: { name: 'x' } })) as { error: { code: string } }).error
        .code,
    ).toBe('RESOURCE_FORBIDDEN'); // viewer-only 403
    expect(
      ((await call(h, 'collections.update', { id: 404, input: { name: 'x' } })) as { error: { code: string } }).error
        .code,
    ).toBe('RESOURCE_FORBIDDEN'); // invisible 404
    ...
    expect(
      ((await call(h, 'collections.deletePlace', { placeId: 404 })) as { error: { code: string } }).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
  });
});
...
describe('create-rpc-host — Wave 3 wiring (files write / collab / member-add)', () => {
  ...
  it('files.create writes bytes, blocks bad extensions + foreign targets, broadcasts file:created', async () => {
    ...
    const good = await call(h, 'files.create', {
      tripId: 1,
      input: { name: 'plan.pdf', content_base64: Buffer.from('hello').toString('base64') },
    });
    expect(good.ok).toBe(true);
    expect(broadcast.mock.calls.some((c) => c[1] === 'file:created')).toBe(true);
    expect(
      (
        (await call(h, 'files.create', { tripId: 1, input: { name: 'evil.exe', content_base64: 'aGk=' } })) as {
          error: { code: string };
        }
      ).error.code,
    ).toBe('BAD_PARAMS');
    expect(
      (
        (await call(h, 'files.create', { tripId: 1, input: { name: 'noext', content_base64: 'aGk=' } })) as {
          error: { code: string };
        }
      ).error.code,
    ).toBe('BAD_PARAMS');
    expect(
      (
        (await call(h, 'files.create', {
          tripId: 1,
          input: { name: 'a.pdf', content_base64: 'aGk=', reservation_id: 999 },
  ...
  it('files.create blocks a demo user while DEMO_MODE is on, but not other members', async () => {
    ...
    try {
      ...
      const denied = await call(
        h,
        'files.create',
        { tripId: 1, input: { name: 'demo.pdf', content_base64: Buffer.from('x').toString('base64') } },
        77,
      );
      ...
      const ok = await call(
        h,
        'files.create',
        { tripId: 1, input: { name: 'ok.pdf', content_base64: Buffer.from('x').toString('base64') } },
        5,
      );
  ...
  it('files link/update/softDelete verify the file is on the trip + same-trip targets', async () => {
    ...
    expect(
      ((await call(h, 'files.createLink', { tripId: 1, fileId: 404, opts: {} })) as { error: { code: string } }).error
        .code,
    ).toBe('RESOURCE_FORBIDDEN');
    expect(
      (
        (await call(h, 'files.createLink', { tripId: 1, fileId: 130, opts: { place_id: 999 } })) as {
          error: { code: string };
        }
      ).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
    ...
    expect(
      ((await call(h, 'files.softDelete', { tripId: 1, fileId: 404 })) as { error: { code: string } }).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
  });
  ...
  it('collab writes delegate + broadcast; service errors map to BAD_PARAMS; addon gated', async () => {
    ...
    expect(
      (await call(h, 'collab.createPoll', { tripId: 1, input: { question: 'Where?', options: ['A', 'B'] } })).ok,
    ).toBe(true);
    expect((await call(h, 'collab.votePoll', { tripId: 1, pollId: 141, optionIndex: 0 })).ok).toBe(true);
    expect(
      ((await call(h, 'collab.votePoll', { tripId: 1, pollId: 141, optionIndex: 9 })) as { error: { code: string } })
        .error.code,
    ).toBe('BAD_PARAMS');
    expect((await call(h, 'collab.createMessage', { tripId: 1, text: 'hi' })).ok).toBe(true);
    expect(
      ((await call(h, 'collab.createMessage', { tripId: 1, text: 'toolong' })) as { error: { code: string } }).error
        .code,
    ).toBe('BAD_PARAMS');
    isAddonEnabled.mockReturnValue(false);
    expect(
      ((await call(h, 'collab.createNote', { tripId: 1, input: { title: 'x' } })) as { error: { code: string } }).error
        .code,
    ).toBe('RESOURCE_FORBIDDEN');
  });
  ...
  it('trips.addMember verifies the target user exists and reports owner-add as joined:false', async () => {
    ...
    expect(
      ((await call(h, 'trips.addMember', { tripId: 1, userId: 12345 })) as { error: { code: string } }).error.code,
    ).toBe('RESOURCE_FORBIDDEN');
  });
});
...
describe('create-rpc-host — Wave 4 wiring (notify / ai)', () => {
  ...
  it('notify.send delegates to notificationService with the plugin_notification event + forced target', async () => {
    ...
    expect((await call(h, 'notify.send', { input: { title: 't', body: 'b', scope: 'trip', targetId: 1 } }, 5)).ok).toBe(
      true,
    );
  });
  ...
  it('ai.complete/extract run under the resolved provider; unconfigured user → BAD_PARAMS', async () => {
    ...
    expect(((await call(h, 'ai.complete', { prompt: 'hi' }, 7)) as { error: { code: string } }).error.code).toBe(
      'BAD_PARAMS',
    );
  });
