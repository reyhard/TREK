...
export class PluginRegistryService {
  ...
  > {
    ...
    return reg.plugins.map((p) => {
      ...
      return {
        ...
        screenshotUrl: p.screenshotUrl?.trim() || (latest ? rawFileUrl(p.repo, latest.commitSha, 'docs/screenshot.png') : null),
  ...
  async detail(id: string): Promise<Record<string, unknown>> {
    ...
    const data = {
      ...
      requiredAddons: latest?.requiredAddons ?? [],
      pluginDependencies: latest?.pluginDependencies ?? [],
      screenshotUrl: entry.screenshotUrl?.trim() || (latest ? rawFileUrl(entry.repo, latest.commitSha, 'docs/screenshot.png') : null),
