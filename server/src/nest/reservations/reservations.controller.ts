...
import { isDemoWriteBlocked } from '../common/demo-write';
import { TransitRouteEndpointUpdateError } from '../common/transitRouteEndpointService';
import {
  ReservationCreateDto,
  ReservationUpdateDto,
  ReservationPositionsDto,
  ReservationTravelersDto,
  TransitRouteEndpointsUpdateDto,
} from './reservations.dto';
import { transitRouteEndpointsUpdateRequestSchema, type TransitRouteEndpointsUpdateRequest } from '@trek/shared';
...
export class ReservationsController {
  constructor(
    ...
    @Optional() private readonly env?: RuntimeEnvService,
  ) {}
  ...
  updateTransitEndpoints(
    ...
    @Param('tripId') tripId: string,
    @Param('id') id: string,
    @Body(new ZodValidationPipe(transitRouteEndpointsUpdateRequestSchema)) body: TransitRouteEndpointsUpdateDto,
    @Headers('x-socket-id') socketId?: string,
  ) {
    if (this.env && isDemoWriteBlocked(this.env, user.email)) {
    ...
    try {
      ...
      this.reservations.notifyBookingChange(tripId, user.id, reservation.title, reservation.type || 'transit');
      return { reservation };
    } catch (err: unknown) {
