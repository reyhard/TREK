...
export class ReservationsService {
  ...
  updateTransitRouteEndpoints(
    ...
    const current = this.getReservation(reservationId, tripId);
    if (!current) {
    ...
    this.db.transaction(() => {
      for (const [role, endpoint] of Object.entries(parsed.data)) {
    ...
    const reservation = this.getReservationWithJoins(reservationId);
    if (!reservation) {
      throw new TransitRouteEndpointUpdateError('RESERVATION_NOT_FOUND', 'Reservation not found.');
    }
    return reservation;
