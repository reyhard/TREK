...
export class TransitMcp {
  ...
  @Tool({
    ...
    inputSchema: {
      ...
      from: transitRouteEndpointsUpdateRequestSchema.shape.from,
      to: transitRouteEndpointsUpdateRequestSchema.shape.to,
    },
    annotations: TOOL_ANNOTATIONS_OPEN_WORLD_NON_IDEMPOTENT,
    access: { group: 'reservations', mode: 'write' },
  })
  async updateTransitRouteEndpoints(
    { tripId, reservationId, from, to }: {
      tripId: number;
    ...
    try {
      const reservation = this.reservations.updateTransitRouteEndpoints(reservationId, tripId, { from, to });
      this.guards.safeBroadcast(tripId, 'reservation:updated', { reservation });
      this.reservations.notifyBookingChange(tripId, ctx.userId, reservation.title, reservation.type || 'transit');
      return ok({ reservation });
    } catch (err) {
