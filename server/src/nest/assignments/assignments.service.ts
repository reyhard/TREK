...
import type { AssignmentTimeRequest, TrekWsPayload, TrekWsTripEventName } from '@trek/shared';
...
import { resolveAssignmentTiming } from '../common/assignmentTiming';
...
export class AssignmentsService {
  ...
  updateTime(id: string | number, placeTime: unknown, endTime: unknown) {
    ...
    if (placeTime === undefined && endTime === undefined) return this.getAssignmentWithPlace(Number(id));

    const requestedPlaceTime = placeTime === '' ? null : placeTime;
    const requestedEndTime = endTime === '' ? null : endTime;

    const request: AssignmentTimeRequest = {};
    if (requestedPlaceTime !== undefined) request.place_time = requestedPlaceTime as string | null;
    if (requestedEndTime !== undefined) request.end_time = requestedEndTime as string | null;
    ...
    this.dbs.transaction(() => {
      this.dbs.run('UPDATE day_assignments SET assignment_time = ?, assignment_end_time = ? WHERE id = ?',
        timing.placeTime ?? (requestedPlaceTime === null ? CLEARED_TIME : current.assignment_time),
        timing.endTime ?? (requestedPlaceTime === null || requestedEndTime === null ? CLEARED_TIME : current.assignment_end_time),
        id);

      // Auto-sort: reorder timed assignments chronologically within the day
      if (requestedPlaceTime) {
