...
export async function buildApp(): Promise<INestApplication> {
  ...
  const pluginBody = express.json({ limit: '10mb', verify: rawBodyKeeper });
  ...
  const isPlugin = (req: Request) =>
    req.path === '/api/plugins' || req.path.startsWith('/api/plugins/');
  ...
  instance.use(function jsonParser(req: Request, res: Response, next: NextFunction) {
    ...
    if (isPlugin(req)) return pluginBody(req, res, next);
