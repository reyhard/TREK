...
function currentPasswordVersion(userId: number): number {
  const row = db.prepare('SELECT password_version FROM users WHERE id = ?').get(userId) as
...
export function getUserByAccessToken(rawToken: string): OAuthTokenInfo | null {
  ...
  `,
    ...
    .get(hash) as
    | (OAuthTokenRow & { username: string; email: string; role: string; current_password_version: number })
    | undefined;
...
export function refreshTokens(
  ...
  if (
    row.user_password_version !== (row as OAuthTokenRow & { current_password_version: number }).current_password_version
  ) {
    writeAudit({
      userId: row.user_id,
      action: 'oauth.token.invalidated_password_version',
...
export function validateAuthorizeRequest(params: AuthorizeParams, userId: number | null): ValidateAuthorizeResult {
  ...
  if (!compatibility.valid) {
    return {
      ...
      error_description:
        compatibility.error === 'invalid_scope'
