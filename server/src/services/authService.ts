...
export function loginUser(body: { email?: string; password?: string; remember_me?: boolean }): {
  ...
  const user = db
    .prepare('SELECT * FROM users WHERE LOWER(email) = LOWER(?) AND COALESCE(is_guest, 0) = 0')
...
export function updateApiKeys(
  ...
  const current = db
    .prepare('SELECT maps_api_key, openweather_api_key, unsplash_api_key FROM users WHERE id = ?')
    .get(userId) as Pick<User, 'maps_api_key' | 'openweather_api_key' | 'unsplash_api_key'> | undefined;
...
export function updateSettings(
  ...
  if (username !== undefined) {
    ...
    const conflict = db
      .prepare('SELECT id FROM users WHERE LOWER(username) = LOWER(?) AND id != ? AND COALESCE(is_guest, 0) = 0')
      .get(trimmed, userId);
  ...
  if (email !== undefined) {
    ...
    const conflict = db
      .prepare('SELECT id FROM users WHERE LOWER(email) = LOWER(?) AND id != ? AND COALESCE(is_guest, 0) = 0')
      .get(trimmed, userId);
...
export function getSettings(userId: number): { error?: string; status?: number; settings?: Record<string, unknown> } {
  const user = db
    .prepare('SELECT role, maps_api_key, openweather_api_key, unsplash_api_key FROM users WHERE id = ?')
...
export function listUsers(excludeUserId: number) {
  ...
  const users = db
    .prepare('SELECT id, username, avatar FROM users WHERE id != ? AND COALESCE(is_guest, 0) = 0 ORDER BY username ASC')
...
export function requestPasswordReset(rawEmail: string, createdIp: string | null): PasswordResetRequestOutcome {
  ...
  const user = db
    .prepare('SELECT id, email, password_hash, oidc_sub FROM users WHERE email = ? AND COALESCE(is_guest, 0) = 0')
    .get(email) as { id: number; email: string; password_hash: string | null; oidc_sub: string | null } | undefined;
