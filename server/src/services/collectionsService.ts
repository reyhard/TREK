...
export function saveFromTripPlaces(
  ...
  for (const placeId of placeIds) {
    const p = db.prepare('SELECT * FROM places WHERE id = ? AND trip_id = ?').get(placeId, tripId) as
      | Record<string, unknown>
      | undefined;
    if (!p) continue;
...
export function updatePlace(
  userId: number,
  placeId: number,
  body: import('@trek/shared').CollectionPlaceUpdateRequest,
  socketId?: string,
): CollectionPlace {
  ...
  if (body.collection_id !== undefined && body.collection_id !== currentCollection) {
    ...
    params.push(ownerOf(body.collection_id));
...
export function copyToTrip(
  ...
  const role =
    (db.prepare('SELECT role FROM users WHERE id = ?').get(userId) as { role: string } | undefined)?.role ?? 'user';
  ...
  for (const s of sources) {
    ...
    const tagIds = db.prepare('SELECT tag_id FROM collection_place_tags WHERE collection_place_id = ?').all(s.id) as {
      tag_id: number;
    }[];
